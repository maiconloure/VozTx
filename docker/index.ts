...
function hasOwnProperty(obj: any, prop: any) {
    return obj.prototype.hasOwnProperty(prop)
}
...
class Recognizer {
     ...
     * @returns true if silence is occured and you can retrieve a new utterance with result method
     */
    acceptWaveformAsync(data: any) {
        return new Promise((resolve, reject) => {
            libvosk.vosk_recognizer_accept_waveform.async(this.handle, data, data.length, function(err: any, result: any) {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
